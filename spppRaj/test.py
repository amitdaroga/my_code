...
def Scraping_page():
    try:
        ...
        email_id=((driver.find_element(By.XPATH,value="//*[text()='Procuring Entity Contact:']")).find_element(By.XPATH,value='..'))#.replace('Procuring Entity Contact:','').split(' ')
        #email_id.remove('')
        email_id=email_id.find_element(By.CLASS_NAME,'data_text').text.replace('[at]','@').replace('[dot]','.').split(' ')
        #print(email_id[1])
        #email_id=(((' '.join(email_id)).replace('[at]','@')).replace('[dot]','.')).split(' ')
        li.append(email_id[1])
        #-----------------------------------------------------------
        address=((driver.find_element(By.XPATH,value="//*[text()='Office Address:']")).find_element(By.XPATH,value='..'))#.replace('Office Address:','').split(' ')
        #address.remove('')
        address=address.find_element(By.CLASS_NAME,'data_text').text.replace(',',' ').strip()
        li.append(address)
        ...
        #_------------------------------------------------------------------------
        tender_type=((driver.find_element(By.XPATH,value="//*[text()='Bid Type']")).find_element(By.XPATH,value='..'))#.replace('Bid Type','').split(' ')
        # tender_type.remove('')
        # li.extend((' '.join(tender_type)).split())
        #print(li)
        tender_type=tender_type.find_element(By.CLASS_NAME,'data_text').text.strip()
        li.append(tender_type)
        #________________________________________________________________________
        ...
        pdf_download(pdf)
        kl=every_downloads_chrome(driver,pdf)
        #driver.get('')
        #driver.close()
...
def insert_data_table(database_rows):
    ...
    sql = f''' INSERT INTO tender_table (Purchaser_Name,Pur_Email,Pur_Add,Tender_Notice_No,TenderType,actualvalue,Bid_Deadline_1,OpeningDate,Tender_Summery,createdOn,currency) VALUES{task}'''
